...
CONFIG_NAME = "isic/isic2018_polyp.yaml"
...
tr_dataset = ISIC2018DatasetFast(data_dir=config['dataset']['training']['params']['data_dir'], mode="tr", one_hot=True)
vl_dataset = ISIC2018DatasetFast(data_dir=config['dataset']['training']['params']['data_dir'], mode="vl", one_hot=True)
te_dataset = ISIC2018DatasetFast(data_dir= config['dataset']['training']['params']['data_dir'], mode="te", one_hot=True)
...
def train(
    ...
    for epoch in range(EPOCHS):
        ...
        for batch, batch_data in tr_iterator:
            ...
            optimizer.zero_grad()
            preds5, preds4, preds3, preds2, preds1 = model(imgs)
            loss5 = criterion(preds5, msks)
            loss4 = criterion(preds4, msks)
            loss3 = criterion(preds3, msks)
            loss2 = criterion(preds2, msks)
            loss1 = criterion(preds1, msks)
            
            loss = loss5 + loss4 + loss3 + loss2 + loss1
            loss.backward()
            optimizer.step()
            
            preds = preds5 + preds4 + preds3 + preds2 +preds1           
...
model = Net(n_channels=config['model']['params']['n_channels'],
